...
- The `eventHandling` setting is now honored. With `eventHandling=false` (the default) no ORM event fires: not entity event methods, not the global `eventHandler`, and not `postNew`. Applications that use ORM events must set `eventHandling: true`, as Adobe ColdFusion and Lucee also require. See [Events](../usage/events.md).
- The third argument of `entityLoadByPK()` is now a struct of load options (`lock`, `timeout`, `skipLocked`, `readOnly`). A boolean third argument (Lucee's `unique`) is still accepted and ignored.
- Criteria `asStruct()` returns date values as ISO 8601 strings (`2024-03-05T10:20:30Z`), like `entityToStruct()` and `entityLoadAsStruct()`.
- The `useDBForMapping` setting now works (it was ignored before). It is off by default, and it never infers foreign keys. See [Using the Database for Mapping](../intro/configuration.md#using-the-database-for-mapping).

### ⭐ Added

- Soft delete: `softDelete="true|active|timestamp"` (and `softDeleteColumn`) on an entity makes `entityDelete()` mark the row deleted instead of removing it, and every load skips deleted rows. See [Soft delete](../modeling/entities.md#soft-delete).
- Automatic timestamps: `autoTimestamp="create"` or `autoTimestamp="update"` on a property sets it on insert, or on every insert and update. See [Automatic Timestamps](../modeling/properties.md#automatic-timestamps).
- `entityEvict()`: remove an entity, or an array of entities, from the session.
- `entityGetReference()`: a lazy reference to an entity by id, without a `SELECT`.
- `entityLoadOrNew()`, `entityLoadOrSave()`: load an entity by id or filter, or create (and save) a new one.
- `entityLoadOrFail()`, `entityLoadByPKOrFail()`: load an entity or throw `orm.notFound`.
- Pessimistic locking: `entityLock()`, the `lock` option of `entityLoadByPK()` and `ormExecuteQuery()` (with `timeout`/`lockTimeout` and `skipLocked`), and `lock()` on `entityCriteria()`. See [Locking](../usage/transactions.md#locking).
- Read-only loads: `ormReadOnly( callback )`, `entityLoadReadOnly()`, and the `readOnly` option of `entityLoad()` and `entityLoadByPK()`.
- `entityCriteria()` bulk terminals: `updateAll( values )` and `deleteAll()` change every matching row with one statement. See [Bulk updates and deletes](../usage/criteria.md#bulk-updates-and-deletes).
- `entityLoadByPK( name, [ ids ] )`: load several entities with one batched query. The array keeps the asked order, with `null` for missing ids.
- `entitySave()` and `entityDelete()` take one entity or an array, and `{ flush : true }` to flush right away.
- `defaultSort` entity annotation (`defaultSort="lastName, firstName desc"`): the order `entityLoad()` and `entityCriteria()` use when none is given. Checked at startup. See [Default sort order](../modeling/entities.md#default-sort-order).
- Functions in criteria paths: `isEq( "year(createdDate)", 2025 )`, `order( "lower(name)" )`, nested calls, literals and `cast()`. See [Functions in paths](../usage/criteria.md#functions-in-paths).
- Named SQL functions: the `sqlFunctions` setting registers SQL templates that HQL and criteria paths call by name, and `ormGetSQLFunctions()` lists them. See [Named SQL Functions](../intro/configuration.md#named-sql-functions).
- `postCommit` event on the entity and the global event handler, fired once a write is committed and never for rolled-back writes. See [postCommit](../usage/events.md#postcommit-after-the-commit).
- `entityToStruct( entityOrArray, [options] )`: entities as structs, mementifier-compatible (`this.memento` defaults, excludes, `neverInclude`, mappers, defaults, profiles), with dotted association paths, cycle safety and ISO 8601 dates. See [Entities as Structs](../usage/structs.md).
...
- The entity-level `where` annotation (`class persistent="true" where="..."`) was ignored. It now restricts every query and load of the entity, including `entityLoadByPK()`.
